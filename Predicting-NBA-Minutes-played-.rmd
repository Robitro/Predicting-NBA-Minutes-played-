...
starters$groupStartPosition <- as.factor(starters$groupStartPosition)
...
```
Data from this project was scraped from the NBA website using NBAstatR, an R package designed to pull data from various basketball databases on the internet. Here we have taken a subset of 165 games of the 1320 games played in the 2021-2022 NBA season pulling every 8 games from the very start to very finish in the regular season. I then removed the data that had players coming off the bench so I could analyze the minutes played of starters.

```{r,fig.dim = c(10,10)}


correlation <- cor(select_if(starters,is.numeric))
...
summary(starters$minExact[starters$groupStartPosition=="C"])
summary(starters$minExact[starters$groupStartPosition=="F"])
summary(starters$minExact[starters$groupStartPosition=="G"])
```
...
min_re <- recipe(minExact ~ pts+oreb+dreb+ast+fg3a+fg3m+blk+pf+tov+fg2a+fg2m+fta+ftm+pctUSG+ortg+drtg+possessions+plusminus+groupStartPosition+pace+tov+pctTOVTeam, data = starter_train) %>% step_dummy(all_nominal_predictors()) %>% step_normalize(all_numeric_predictors())
