...
min_re <- recipe(minExact ~ pts +oreb+dreb+ast+fg3a+fg3m+blk+pf+tov+fg2a+fg2m+fta+ftm+pctUSG+ortg+drtg+possessions+plusminus+groupStartPosition+pace+tov, data = starter_train) %>% step_dummy(all_nominal_predictors()) %>% step_normalize(all_numeric_predictors())
...
bt_wf <- workflow() %>% add_model(bt %>% set_args(trees = tune())) %>% add_formula(minExact ~ pts +oreb+dreb+ast+fg3a+fg3m+blk+pf+tov+fg2a+fg2m+fta+ftm+pctUSG+ortg+drtg+possesions+plusminus+groupStartPosition+pace+tov)
