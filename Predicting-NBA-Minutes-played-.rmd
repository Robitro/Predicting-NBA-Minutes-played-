...
```{r}
ggplot(player,aes(minExact, treb, group=minExact, fill=oreb))+ geom_point()+facet_wrap(~groupStartPosition)
...
best_elas
```
It appears that Elastic Net Model 95, a lasso regression model with a penalty of .2783 produces an RMSE of 1.5637. It is interesting as I would've though a mixture of lasso and ridge would preform best.
...
```
 KNN Model 6, A 28 nearest neighbor model weighted with the biweight function produced the lowest RMSE with 3.3310.
...
best_boost
```
Boosted tree model 20, a 1000 tree model with a tree depth of 2 produced the lowest RMSE with .3665.
...
```
Random Forest model 24, a model that used all 20 predictors to use its trees and had a minimum of 3 nodes produced the lowest RMSE with .4494.


# Congratulations Boosted Tree Model 20!!! 

Our Boosted tree Model 20, a 1000 tree model with a tree depth of 2 performed best out of the 812 total models across our four model types. With an RMSE value of .3665, it was .0829 lower than our best random forest model, 1.197 lower RMSE than the best elastic net model, and a huge 2.9645 lower RMSE than our best KNN model. Our Boosted tree model fit a 2 level pruned decision tree and weighted features it found important. Let's finalize our workflow and fit our model to the testing set.

### Fitting Boosted Tree Model 20 to our testing set
```{r}

bt_wf_f <- finalize_workflow(bt_wf,best_boost)

min_train_fit <- fit(bt_wf_f, data = player_train)

min_test_fit <- fit(bt_wf_f, data = player_test)

player_test_pred <- augment(min_test_fit,player_test)
player_train_pred <- augment(min_train_fit,player_train)
...
```
Our model returned an RSQ value of .9999 and a  Root Mean Squared Error of .0965 on our test data, and an RSQ value of .9997 and a RMSE of .1639.   
...
rmse(min_pred, truth = minExact, estimate = .pred)
