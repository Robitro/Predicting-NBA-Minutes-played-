...
summary(player$minExact[starters$groupStartPosition=="C"])
summary(player$minExact[starters$groupStartPosition=="F"])
summary(player$minExact[starters$groupStartPosition=="G"])
summary(player$minExact[starters$groupStartPosition=="Bench"])
```
...
```{r}
ggplot(player,aes(minExact,pts))+ geom_point(aes(fill = cut(fga, c(0.0,5, 10,15,20,35))),size = 2) + facet_wrap(~groupStartPosition) + #scale_fill_manual("Field Goals Attempted", values = c("0-5" = "Green", "5-10" = "Purple", "10-15" = "Gold", "15-20"="Red","20-35" = ""))

```
Basketball is grouped into 3 groups (guards, forwards, centers) and 5 positions (point guard, shooting guard, small forward, power forward, and center). As we can see, the more minutes a player plays there is an upward trajectory in points scored as well as shot attempts. This makes sense as the only way to score points is to shoot field goals or shoot free throws, and to shoot free-throws you need to be fouled which often occurs when a player is trying to make a shot.
```{r}
...
```{r}
ggplot(player, aes(minExact,treb, group=minExact, fill=oreb))+ geom_boxplot() + scale_x_discrete(breaks = seq(0,50,5))+facet_wrap(~groupStartPosition)
...
min_re <- recipe(minExact ~ pts+oreb+dreb+ast+fg3a+fg3m+blk+pf+tov+fg2a+fg2m+fta+ftm+pctUSG+ortg+drtg+possessions+plusminus+pace+tov+pctTOVTeam, data = player_train) %>% step_dummy(all_nominal_predictors()) %>% step_normalize(all_numeric_predictors())
...
```
The first model we will fit will be elastic net regression, a regularized regression model which combines ridge and lasso coefficients to fit a linear model on our data.
...
```
k nearest neighbors works by looking at the k closest data points and works to assign a value similar to those data points.

## Boosted Trees
```{r}
bt <- boost_tree() %>% set_engine("xgboost") %>% set_mode("regression")

bt_wf <- workflow() %>% add_model(bt %>% set_args(trees = tune())) %>% add_recipe(min_re)
...
rf_wf <- workflow() %>% add_model(rf %>% set_args(mtry = tune(), trees = tune(),min_n = tune())) %>% add_recipe(min_re)

rand_grid <- grid_regular(mtry(range= c(1,20)), trees(range = c(1,1000)), min_n(range = c(1,5)),levels = 8)


```

```{r,eval=FALSE}
tune_tree <- tune_grid(bt_wf, resamples = starter_folds, grid = tree_grid, metrics = metric_set(roc_auc) )
